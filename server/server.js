  ...
  getAuth()
    ...
    .then(async (decodedUser) => {
      ...
      if (user) {
        ...
        if (!user.google_auth) {
          return res.status(403).json({
            error:
              'This email was already signed up without google. Please log in with password to access the account',
          });
        }
      } else {
        //signup
    ...
    .catch((err) => {
      return res.status(500).json({
        error:
          'Failed to authenticate you with Google. Try signing up with some other Google account',
      });
    });
});
